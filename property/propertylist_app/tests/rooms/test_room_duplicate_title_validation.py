...
def test_duplicate_room_title_returns_clear_title_validation_error(django_user_model):
    landlord_one = django_user_model.objects.create_user(
        username="duplicate_title_landlord_one",
        email="duplicate_title_landlord_one@example.com",
        password="testpass123",
    )
    landlord_two = django_user_model.objects.create_user(
        username="duplicate_title_landlord_two",
        email="duplicate_title_landlord_two@example.com",
        password="testpass123",
    )
    ...
    payload = {
        ...
        "description": (
            ...
            "modern furnishings, fast broadband, secure entry, excellent transport "
            "links, nearby shops, a comfortable shared kitchen, useful storage space, "
            "quiet surroundings, and easy access to the city centre for work or study."
        ),
        "location": "SW1A 1AA",
    ...
    second_response = second_client.post(
        url,
        {**payload, "title": "BRIGHT DOUBLE ROOM IN SHARED FLAT"},
        format="json",
    )

    assert second_response.status_code == status.HTTP_400_BAD_REQUEST, second_response.data
    assert second_response.data["code"] == "validation_error"
    assert second_response.data["field_errors"]["title"] == [
